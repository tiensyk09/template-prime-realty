...
export default function PostSidebarClient({ recentUpdates = [], suggestedProducts = [] }) {
  ...
  return (
    <aside style={{ display: 'flex', flexDirection: 'column', gap: '24px', position: 'sticky', top: '130px', alignSelf: 'flex-start' }}>
      ...
      {/* 1. Contact CTA - Prime Realty */}
      <div style={{
        background: 'linear-gradient(135deg, #c9a24b 0%, #0a4f26 100%)',
        color: '#ffffff',
        borderRadius: '16px',
        padding: '24px',
        ...
        <p style={{ fontSize: '13px', lineHeight: 1.5, opacity: 0.9, margin: '0 0 16px 0' }}>
          Liên hệ trực tiếp để được chuyên viên Prime Realty tư vấn bất động sản tận tình, chu đáo.
        </p>
      ...
      {suggestedProducts.length > 0 && (
        <div style={{
          ...
          <h3 style={{ fontSize: '15px', fontWeight: 800, color: '#0d2a4d', marginBottom: '16px', display: 'flex', alignItems: 'center', gap: '8px', borderBottom: '2px solid #f0faf4', paddingBottom: '8px' }}>
                ...
                <div key={prod.id} style={{ display: 'flex', flexDirection: 'column', gap: '8px', paddingBottom: '12px', borderBottom: '1px solid #f3f4f6' }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-between', minWidth: 0 }}>
                      ...
                      <div>
                        <div style={{ display: 'flex', alignItems: 'baseline', gap: '4px', flexWrap: 'wrap' }}>
                          <span style={{ fontSize: '13.5px', fontWeight: 700, color: '#c9a24b' }}>
                  ...
                  <button
                    ...
                    style={{
                      ...
                      color: addingId === prod.id ? '#fff' : '#c9a24b',
                      fontWeight: 700,
                      fontSize: '11.5px',
                      cursor: 'pointer',
                      display: 'flex',
                    ...
                    onMouseLeave={(e) => {
                      if (addingId !== prod.id) {
                        e.currentTarget.style.background = '#f6f2e9';
            ...
            {recentUpdates.map(u => (
              <div key={u.id} style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
                <div style={{ width: '60px', height: '45px', flexShrink: 0, borderRadius: '6px', overflow: 'hidden', backgroundColor: '#f3f4f6' }}>
                  <img
                    src={u.image || 'https://images.unsplash.com/photo-1588776814546-1ffcf47267a5?auto=format&fit=crop&w=400&q=80'}
                    alt={u.title}
