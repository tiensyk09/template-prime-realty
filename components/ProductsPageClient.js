...
export default function ProductsPageClient({
  ...
  const filteredProducts = products.filter(prod => {
    // Price Filter
    if (priceFilter === 'under-500' && prod.price >= 2000000000) return false;
    if (priceFilter === '500-1000' && (prod.price < 2000000000 || prod.price > 5000000000)) return false;
    if (priceFilter === '1000-2000' && (prod.price < 5000000000 || prod.price > 10000000000)) return false;
    if (priceFilter === 'over-2000' && prod.price <= 10000000000) return false;
    ...
    <div style={{ backgroundColor: '#f5f7fa', minHeight: '100vh', padding: '24px 0 60px' }}>
      <div style={{ maxWidth: '1280px', margin: '0 auto', padding: '0 20px' }}>
        ...
        <div style={{ fontSize: '13px', color: '#6b7280', marginBottom: '20px', display: 'flex', gap: '6px', alignItems: 'center' }}>
          ...
          <span>/</span>
          <Link href="/products" style={{ color: '#374151', textDecoration: 'none' }}>Dịch vụ</Link>
          {activeCategory && (
            <>
              <span>/</span>
              <span style={{ color: '#c9a24b', fontWeight: 600 }}>{activeCategory.name}</span>
            </>
          )}
          {searchQ && (
            <>
        ...
        <div className="lc-mobile-categories-scroll" style={{ display: 'none', gap: '8px', overflowX: 'auto', paddingBottom: '16px', margin: '0 -20px 16px -20px', paddingLeft: '20px', paddingRight: '20px', scrollbarWidth: 'none', WebkitOverflowScrolling: 'touch' }}>
          <Link
            ...
            style={{
              ...
              fontWeight: 700,
              textDecoration: 'none',
              background: !activeCategory ? '#c9a24b' : '#ffffff',
              color: !activeCategory ? '#ffffff' : '#374151',
              border: '1px solid ' + (!activeCategory ? '#c9a24b' : '#e5e7eb'),
            ...
            return (
              <Link
                ...
                style={{
                  ...
                  fontWeight: 700,
                  textDecoration: 'none',
                  background: isActive ? '#c9a24b' : '#ffffff',
                  color: isActive ? '#ffffff' : '#374151',
                  border: '1px solid ' + (isActive ? '#c9a24b' : '#e5e7eb'),
                  boxShadow: '0 2px 6px rgba(0,0,0,0.03)'
                }}
              >
                {cat.icon || '📦'} {cat.name}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '280px 1fr', gap: '28px' }} className="products-layout-grid">
          ...
          <aside className="lc-desktop-sidebar" style={{ display: 'flex', flexDirection: 'column', gap: '20px', alignSelf: 'start' }}>
            ...
            <div style={{ background: '#ffffff', borderRadius: '16px', padding: '24px', border: '1px solid #e5e7eb', boxShadow: '0 4px 16px rgba(0,0,0,0.02)' }}>
              <h3 style={{ fontSize: '16px', fontWeight: 800, borderBottom: '1px solid #f3f4f6', paddingBottom: '12px', marginBottom: '16px', color: '#0d2a4d', display: 'flex', alignItems: 'center', gap: '8px' }}>
                ...
                <li>
                  <Link
                    ...
                    style={{
                      ...
                      fontSize: '13.5px',
                      color: !activeCategory ? '#ffffff' : '#374151',
                      background: !activeCategory ? '#c9a24b' : 'transparent',
            ...
            <div style={{ background: '#ffffff', borderRadius: '16px', padding: '24px', border: '1px solid #e5e7eb', boxShadow: '0 4px 16px rgba(0,0,0,0.02)' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                {[
                  ...
                  { value: 'under-500', label: 'Dưới 2 tỷ' },
                  { value: '500-1000', label: '2 - 5 tỷ' },
                  { value: '1000-2000', label: '5 - 10 tỷ' },
                  { value: 'over-2000', label: 'Trên 10 tỷ' }
                ].map((opt) => (
                  <label key={opt.value} style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '13.5px', color: '#4b5563', cursor: 'pointer' }}>
                    <input
                      ...
                      style={{ accentColor: '#c9a24b', width: '16px', height: '16px' }}
                    />
                    <span>{opt.label}</span>
                  </label>
                ))}
          ...
          <section style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            ...
            <div style={{ background: '#ffffff', borderRadius: '16px', padding: '16px 24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', border: '1px solid #e5e7eb', boxShadow: '0 4px 16px rgba(0,0,0,0.02)', flexWrap: 'wrap', gap: '12px' }}>
              <div style={{ fontSize: '14.5px', color: '#374151', fontWeight: 500 }}>
                Tìm thấy <strong>{filteredProducts.length}</strong> dịch vụ
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                ...
                <button
                  ...
                  style={{
                    ...
                    padding: '8px 16px',
                    borderRadius: '8px',
                    border: '1.5px solid ' + (showMobileFilters ? '#c9a24b' : '#e5e7eb'),
                    fontSize: '13.5px',
                    fontWeight: 700,
                    cursor: 'pointer',
                    backgroundColor: showMobileFilters ? '#f6f2e9' : '#ffffff',
                    color: showMobileFilters ? '#c9a24b' : '#374151'
                  }}
                >
            ...
            {showMobileFilters && (
              ...
              >
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }} className="lc-mobile-filters-grid">
                  ...
                  {/* Price filter column */}
                  <div>
                    <h4 style={{ fontSize: '14px', fontWeight: 800, color: '#0d2a4d', margin: '0 0 12px 0' }}>Khoảng giá</h4>
                  ...
                  <div>
                    <h4 style={{ fontSize: '14px', fontWeight: 800, color: '#0d2a4d', margin: '0 0 12px 0' }}>Đánh giá</h4>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                      {[
                        { value: 'all', label: 'Tất cả đánh giá' },
                        { value: '5', label: '⭐⭐⭐⭐⭐ 5 Sao' },
                ...
                <div style={{ display: 'flex', gap: '10px', borderTop: '1px solid #f3f4f6', paddingTop: '12px' }}>
                  <button
                    ...
                    style={{ flex: 1, padding: '8px', background: '#c9a24b', color: '#fff', border: 'none', borderRadius: '20px', fontWeight: 700, fontSize: '13px', cursor: 'pointer' }}
                  >
                    Áp dụng bộ lọc
                  </button>
                  <button
            ...
            {filteredProducts.length === 0 ? (
              <div style={{ background: '#ffffff', borderRadius: '16px', padding: '80px 20px', textAlign: 'center', border: '1px solid #e5e7eb', boxShadow: '0 4px 16px rgba(0,0,0,0.02)' }}>
                ...
                <h3 style={{ fontSize: '18px', fontWeight: 800, color: '#0d2a4d', marginBottom: '8px' }}>Không tìm thấy dịch vụ nào</h3>
                <p style={{ color: '#6b7280', fontSize: '14px', margin: '0 0 20px 0' }}>Vui lòng thay đổi khoảng giá, đánh giá hoặc danh mục để tìm thấy bất động sản phù hợp.</p>
                <button
                  onClick={clearFilters}
                  style={{ background: '#c9a24b', color: '#ffffff', border: 'none', padding: '10px 24px', borderRadius: '30px', fontWeight: 700, cursor: 'pointer', fontSize: '13.5px' }}
                >
                      ...
                      >
                        ...
                        <Link href={`/products/${prod.slug}`} style={{ textDecoration: 'none', color: 'inherit', display: 'block', padding: '16px' }}>
                          ...
                          <div style={{ minHeight: '84px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
                            <div>
                              {prod.brand && (
                                <span style={{ fontSize: '11px', textTransform: 'uppercase', color: '#c9a24b', fontWeight: 700, display: 'block', marginBottom: '4px' }}>
                                  {prod.brand}
                                </span>
                              )}
                              <h4 style={{ fontSize: '13.5px', fontWeight: 700, color: '#0d2a4d', margin: 0, display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden', height: '36px', lineHeight: '1.45' }}>
                                {prod.name}
                              </h4>
                            </div>
                          </div>
                          ...
                          <div style={{ marginTop: '12px' }}>
                            <div style={{ display: 'flex', alignItems: 'baseline', gap: '4px', flexWrap: 'wrap' }}>
                              <span style={{ fontSize: '15px', fontWeight: 800, color: '#c9a24b' }}>
                        ...
                        <div style={{ padding: '0 16px 16px', marginTop: 'auto' }}>
                          <button
                            ...
                            style={{
                              ...
                              borderRadius: '24px',
                              border: 'none',
                              background: addingId === prod.id ? '#10b981' : '#f6f2e9',
                              color: addingId === prod.id ? '#ffffff' : '#c9a24b',
                              fontWeight: 700,
                              fontSize: '13px',
                              cursor: 'pointer',
                              transition: 'all 0.2s',
                          ...
                          >
                            {addingId === prod.id ? '✓ Đã thêm!' : 'Đặt dịch vụ'}
                          </button>
                        </div>
                      </div>
                    );
                      ...
                      return (
                        <button
                          ...
                          style={{
                            ...
                            background: isCurrent ? '#c9a24b' : '#ffffff',
